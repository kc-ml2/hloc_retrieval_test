...
if __name__ == "__main__":
    ...
    jsonl_file = gzip.open(val_unseen_gz_file)
    reader = jsonlines.Reader(jsonl_file)
    len_eng_obj = 0
    val_unseen_scan_list = []
    eng_instruction_list_by_scan = []
    for obj in reader:
        # if obj["language"] == "en-IN" or obj["language"] == "en-US":
        if obj["scan"] in val_unseen_scan_list:
            # scan_idx = val_seen_scan_list.index(obj["scan"])
            # eng_instruction_list_by_scan[scan_idx].append(obj["instruction"])
            pass
        else:
            val_unseen_scan_list.append(obj["scan"])
            # eng_instruction_list_by_scan.append([obj["instruction"]])
        # len_eng_obj = len_eng_obj + 1
    # print(len_eng_obj)
    # input()
    print(len(val_unseen_scan_list))
    input()
    print(val_unseen_scan_list)
    input()

    jsonl_file = gzip.open(test_gz_file)
    reader = jsonlines.Reader(jsonl_file)
    len_eng_obj = 0
    test_scan_list = []
    eng_instruction_list_by_scan = []
    for obj in reader:
        # if obj["language"] == "en-IN" or obj["language"] == "en-US":
        if obj["scan"] in test_scan_list:
            # scan_idx = val_seen_scan_list.index(obj["scan"])
            # eng_instruction_list_by_scan[scan_idx].append(obj["instruction"])
            pass
        else:
            test_scan_list.append(obj["scan"])
            # eng_instruction_list_by_scan.append([obj["instruction"]])
        # len_eng_obj = len_eng_obj + 1
    # print(len_eng_obj)
    # input()
    print(len(test_scan_list))
    input()
